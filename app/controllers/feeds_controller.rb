class FeedsController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      if save_feed_and_apple_connection(context: validation_context)
        @feed.copy_media
        @feed.podcast&.publish!
        format.html { redirect_to podcast_feed_path(@podcast, @feed), notice: t(".success", model: "Feed") }
      else
  ...
  def save_feed_and_apple_connection(context: nil)
    saved = false

    Feed.transaction do
      if @feed.save(context: context) && save_apple_connection
        saved = true
      else
