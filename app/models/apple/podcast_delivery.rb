...
module Apple
  class PodcastDelivery < ApplicationRecord
    ...
    has_one :apple_sync_log, -> { podcast_deliveries.apple }, foreign_key: :feeder_id, class_name: "Apple::SyncLog", dependent: :destroy
    ...
    def self.upsert_podcast_delivery(podcast_container, row)
      ...
      (pd, action) =
        if (delivery = with_deleted.where(episode_id: podcast_container.episode.id,
          ...
          delivery.update(status: delivery_status, updated_at: Time.now.utc)
      ...
      Apple::SyncLog.log!(feeder_id: pd.id, feeder_type: :podcast_deliveries, external_id: external_id, api_response: row)
