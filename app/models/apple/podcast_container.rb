...
module Apple
  class PodcastContainer < ActiveRecord::Base
    ...
    def self.create_podcast_containers(api, episodes_to_sync)
      episodes_to_create = episodes_to_sync.reject { |ep| ep.podcast_container.present? }

      api_resp =
        api.bridge_remote("createPodcastContainers",
                          Apple::PodcastContainer.create_podcast_containers_bridge_params(episodes_to_create))
      ...
      new_containers_response.map do |row|
        ...
        external_id = row.dig("podcast_container_response", "data", "id")
    ...
    def self.create_podcast_containers_bridge_params(episodes_to_sync)
      episodes_to_sync.
        map do |ep|
        {
          episode_id: ep.id,
