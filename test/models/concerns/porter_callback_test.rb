...
describe PorterCallback do
  it "handles non-porter messages" do
    assert_nil TestCallback.porter_callback_job_id({})
    assert_nil TestCallback.porter_callback_status({})
    assert_nil TestCallback.porter_callback_time({})
  end

  it "parses porter job received callback messages" do
    porter_received_callback = build(:porter_job_received)
    assert_equal TestCallback.porter_callback_job_id(porter_received_callback), "the-job-id"
    assert_equal TestCallback.porter_callback_status(porter_received_callback), "processing"
    assert_equal TestCallback.porter_callback_time(porter_received_callback), Time.parse("2012-12-21T12:34:56Z")
  end

  it "ignores porter task result callback messages" do
    task_result_callback = build(:porter_task_result)
    assert_equal TestCallback.porter_callback_job_id(task_result_callback), "the-job-id"
    assert_nil TestCallback.porter_callback_status(task_result_callback)
  end

  it "ignores porter task error callback messages" do
    task_error_callback = build(:porter_task_error)
    assert_equal TestCallback.porter_callback_job_id(task_error_callback), "the-job-id"
    assert_nil TestCallback.porter_callback_status(task_error_callback)
  end

  it "parses porter ingest failed callback messages" do
    porter_404_callback = build(:porter_job_ingest_failed)
    assert_equal TestCallback.porter_callback_job_id(porter_404_callback), "the-job-id"
    assert_equal TestCallback.porter_callback_status(porter_404_callback), "error"
    assert_equal TestCallback.porter_callback_time(porter_404_callback), Time.parse("2012-12-21T12:34:56Z")
  end

  it "parses porter job failed callback messages" do
    porter_error_callback = build(:porter_job_failed)
    assert_equal TestCallback.porter_callback_job_id(porter_error_callback), "the-job-id"
    assert_equal TestCallback.porter_callback_status(porter_error_callback), "error"
    assert_equal TestCallback.porter_callback_time(porter_error_callback), Time.parse("2012-12-21T12:34:56Z")
  end

  it "parses porter successful callback messages" do
    porter_success_callback = build(:porter_job_results)
    assert_equal TestCallback.porter_callback_job_id(porter_success_callback), "the-job-id"
    assert_equal TestCallback.porter_callback_status(porter_success_callback), "complete"
    assert_equal TestCallback.porter_callback_time(porter_success_callback), Time.parse("2012-12-21T12:34:56Z")
  end
