...
class Integrations::EpisodeDeliveryStatusTest < ActiveSupport::TestCase
  ...
  describe "#measure_asset_processing_duration" do
    ...
    it "returns nil when no status has uploaded: true and delivered: false" do
      create(:apple_episode_delivery_status, episode: episode, uploaded: false, delivered: false, created_at: 1.hour.ago)
      assert_nil episode.measure_asset_processing_duration
    end
    ...
    it "uses the oldest uploaded && !delivered status for duration" do
    ...
    it "returns nil when delivery is complete" do
      # Once delivered, there are no uploaded && !delivered records
      create(:apple_episode_delivery_status, episode: episode, uploaded: true, delivered: true, created_at: 1.hour.ago)
