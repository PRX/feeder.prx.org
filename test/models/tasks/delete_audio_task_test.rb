...
describe Tasks::DeleteAudioTask do
  let(:task) { create(:delete_audio_task) }
  ...
  it 'can start the job' do
    ...
    task.stub(:get_account_token, "token") do
      task.start!
      task.options[:source].must_match /s3:\/\/test-prx-feed\/jjgo\/ba047dce-9df5-4132-a04b-31d24c7c55a(\d+)\/AR0328segmentA.mp2/
    end
  end
