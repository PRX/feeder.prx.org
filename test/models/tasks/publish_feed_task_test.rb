...
describe Tasks::PublishFeedTask do
  ...
  it 'can start the job' do
    ...
    task.options[:destination].must_equal 's3://test-prx-feed/jjgo/feed-rss.xml?x-fixer-public=true'
  end
  ...
  it 'determines a destination url' do
    ...
    url.must_equal 's3://test-prx-feed/jjgo/feed-rss.xml?x-fixer-public=true'
  end
end
